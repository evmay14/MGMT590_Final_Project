...
from app.common import (
    apply_global_style, download_dataframe_button, get_risk_engine, load_phase3_reports, load_splits_cached,
...
with tab_learning:
    st.caption("Learning curves are recomputed on demand (they require refitting across training-set sizes) — this may take a moment.")
    ...
    if engine is not None:
        with st.spinner(f"Computing learning curve for {model_utils.MODEL_DISPLAY_NAMES[learning_key]}..."):
            X_train, _, _, y_train, _, _ = load_splits_cached()
            fig = model_utils.plot_learning_curve_chart(engine.pipeline, X_train, y_train, title=model_utils.MODEL_DISPLAY_NAMES[learning_key])
        st.pyplot(fig)
